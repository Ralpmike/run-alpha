...
const AboutCard = ({ title, text, buttonText, imageUrl, reverse }) => (
  <div className={`flex ${reverse ? "flex-row-reverse bg-gray-200" : ""} items-center flex-wrap lg:flex-nowrap gap-8 mb-16 2xl:px-56 lg:px-36 px-6 md:py-12`}>
    <motion.div
      ...
      <p className="text-sm tracking-[2] sm:text-base xl:text-lg leading-relaxed text-justify">{text}</p>
      <button className="group hover:bg-alpha hover:rounded-full hover:pr-4 font-bold px-3 py-4 mt-8 italic text-sm hover:text-white transition-all duration-300 flex items-center gap-4 motion-button">
        {buttonText}
